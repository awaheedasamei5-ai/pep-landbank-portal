...
- [ ] The KYC capture modal (§6.4) actually saves real values onto the lead — confirm a filled-in field persists and is readable on a reload, not just displayed transiently.
- [ ] Request → KYC → generate a real contract against a published template with a missing custom field — confirm the unresolved-token guard blocks generation with a plain-English list of exactly what's missing.
- [ ] Generate successfully — confirm a `contract_generations` row exists with a real `content_snapshot`, and that a later edit to the template's live content does NOT change what re-opening this past generation shows.
- [ ] Progress rail shows the correct current step at each stage of a real request's lifecycle.
...
8. Generation-time snapshot integration into the existing `generateAndRecordContract` (§8) — the real integrity fix, now straightforward since items 5-7 already built the resolver + renderer this needs. **NEXT UP.**
