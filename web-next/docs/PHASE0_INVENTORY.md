...
**First real cutover done 2026-09-03 (staging only):** `payments_ins`/`payments_upd`/`payments_del` now read `has_permission('payments.manage')` instead of `(my_key()='elias' OR my_role()='manager')`. Verified two ways: the existing parity suite still passes unchanged, and a new `tests.run_payments_cutover_behavior_tests()` (2/2) exercises the *actual* `payments` table with a real throwaway lead+payment — elias's insert succeeds, emmanuel's throws `42501` (`insufficient_privilege`), both under real RLS enforcement (JWT impersonation of their real profiles, not a simulated check) — then cleans up its own fixture rows (confirmed zero leftover afterward). `payments_sel`/`payments_client_sel` untouched — that's the separate `ops.view_all` grant.

**Deliberately not done here — a separate, one-policy-at-a-time follow-up:** the remaining ~4 policies (`contracts_ins`, `alloc_ins/upd`, `payments_sel`/`contracts_sel`/`site_visits_sel`/`site_visits_del` → `ops.view_all`). Each is business-critical and already working in production; migrate and verify individually, matching the payments precedent above, not as one bulk rewrite. Also not done: porting any of this (`permissions`/`role_permissions`/`staff_permission_overrides`/`has_permission`/the payments cutover) to production, or any `web-next` UI/DataSource wiring (nothing reads this model yet — it exists purely as verified foundation).
...
5. ~~First RLS cutover~~ — done (§4): `payments_ins`/`upd`/`del` → `has_permission('payments.manage')`, verified against the real table (2/2 behavioral), zero leftover fixture data.
6. Continue Phase 1: cut over the remaining ~4 policies one at a time (`contracts_ins`, `alloc_ins/upd`, the `ops.view_all` set), idempotent-RPC review, error-contract standardization. Live-mode manager sign-in through the real login screen is still unexercised — needs a real manager `auth.users` account, which is account creation (out of scope here) rather than a schema task.
