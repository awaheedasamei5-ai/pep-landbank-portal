...
7. Next: a `web-next` permission-management admin screen (grant/revoke UI calling the two new RPCs), idempotent-RPC review, error-contract standardization, planning the actual production cutover (staging-only today, by design). Live-mode manager sign-in through the real login screen is still unexercised — needs a real manager `auth.users` account, which is account creation (out of scope here) rather than a schema task.
