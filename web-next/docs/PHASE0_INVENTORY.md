...
**Deliberately not done here — a separate, one-policy-at-a-time follow-up:** swapping any *existing* RLS policy's hardcoded array check for a call to `has_permission(...)`. Each of the ~7 policies above is business-critical (payments, contracts, allocations) and already working in production; migrating them should happen individually, verified against this same parity suite each time, not as one bulk rewrite. Also not done: porting `permissions`/`role_permissions`/`staff_permission_overrides` to production (staging-only until a cutover is actually planned), and any `web-next` UI/DataSource wiring (nothing reads this model yet — it exists purely as verified foundation).
...
5. Continue Phase 1: the one-policy-at-a-time RLS cutover to `has_permission()` (deliberately deferred, §4), idempotent-RPC review, error-contract standardization. Live-mode manager sign-in through the real login screen is still unexercised — needs a real manager `auth.users` account, which is account creation (out of scope here) rather than a schema task.
