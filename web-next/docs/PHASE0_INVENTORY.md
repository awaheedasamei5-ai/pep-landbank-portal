...
**Correction, same day:** `index.html` was found to already have a real, richer task system on the same `schedule_items` table (`apiInsertTask`/`apiUpdateTaskStatus`/`mapTask`) plus its own `task_events` audit table (assigned/started/blocked/unblocked/awaiting_approval/approved/done/cancelled/reopened) and SMS+push notification on assignment — an initial grep for the literal string "Task Board" missed all of it since legacy never calls it that internally. **Real bug found while investigating:** `apiUpdateTaskStatus` writes `status='blocked'`/`status='awaiting_approval'` for those two task states, but `schedule_items_status_check` (confirmed live on both projects) only allows `open/in_progress/done/cancelled/rescheduled` — selecting "Blocked" or "Awaiting Approval" on a real task in production would throw a raw constraint-violation error. Confirmed via a rolled-back INSERT on staging. Zero rows exist in `task_events` on production, so this has never actually been hit by a real user — flagged, not fixed (a production CHECK constraint change needs its own go-ahead). `web-next`'s Task Board never writes those two values, so it isn't affected.
