...
**Deliberately not done here:** porting any of this (`permissions`/`role_permissions`/`staff_permission_overrides`/`has_permission`/all four cutovers) to production — staging-only until a real cutover to production is planned and approved turn-by-turn (matches the standing production-write rule). Also not done: any `web-next` UI/DataSource wiring (nothing reads this model yet — it exists purely as verified foundation) and a permission-management admin screen (grant/revoke UI) — real follow-up work, not started.
...
5. ~~All four RLS cutovers~~ — done (§4): `payments.manage`, `contracts.generate`, `allocations.manage`, `ops.view_all` — every real hardcoded staff-key array this session found is gone from RLS policy text. 33/33 pgTAP assertions passing across 6 suites.
6. Continue Phase 1: a permission-management admin screen (grant/revoke UI — nothing reads the new model from `web-next` yet), idempotent-RPC review, error-contract standardization, planning the actual production cutover (staging-only today, by design). Live-mode manager sign-in through the real login screen is still unexercised — needs a real manager `auth.users` account, which is account creation (out of scope here) rather than a schema task.
