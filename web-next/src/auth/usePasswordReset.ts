...
export function useSendResetCode() {
  return useMutation({
    mutationFn: async (email: string) => {
      ...
      if (error) throw friendlyErrorObj(error.message || 'Could not send code.');
    },
  });
}
...
export function useConfirmReset() {
  return useMutation({
    mutationFn: async ({ email, code, password }: { email: string; code: string; password: string }) => {
      ...
      if (verifyErr) throw friendlyErrorObj(verifyErr.message || 'That code is invalid or expired.');
      const { error: updErr } = await client.auth.updateUser({ password });
      if (updErr) throw friendlyErrorObj(updErr.message || 'Could not set new password.');
