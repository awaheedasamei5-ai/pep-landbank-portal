...
// A real circular progress ring (not one segment per day, like Attendance's
// SegmentedGauge -- appropriate there for a ~26-day monthly tally, too busy
// for a 20-day annual quota) -- remaining/total as one continuous arc, days
// remaining centered. Requested directly: "A Leave Balance element styled
// as a circular/donut progress indicator... not a plain number."
//
// `reserved` and `confirmedUsed` are deliberately two different numbers
// (user correction 2026-09-05): reserved is every planned/pending/approved
// day protecting the annual cap the moment it's requested; confirmedUsed is
// only days that have actually passed AND the staff member confirmed they
// took (see leaveLogic.ts's leaveDaysReserved/leaveDaysConfirmedUsed). The
// ring itself still tracks remaining-of-total (the entitlement, which reads
// off reserved) -- confirmedUsed is a separate real stat, not the same
// number relabeled.
