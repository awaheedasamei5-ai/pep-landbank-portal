...
//
// Also serves as Master Pipeline (`/dashboard/pipeline`) -- per the user's
  ...
  return (
    ...
    <div className={`${styles.wrap} ${hasDetailOpen ? `${styles.wrapHiddenMobile} ${styles.wrapWithDrawer}` : ''}`}>
      ...
      {profile?.role === 'manager' && (
        <button type="button" className={styles.archivedLink} onClick={() => navigate('/dashboard/pipeline/archived')}>
          View archived leads →
        </button>
      )}
    </div>
