...
export function PipelineDetailScreen() {
  ...
  // agent-scoped /dashboard/pipeline list -- which for a manager shows few
  ...
  // Path-mapping adjustment (not a logic change): this shell has one
  // unified /dashboard/pipeline route for both My/Master Pipeline (real
  // RLS scopes it per role, no separate /dashboard/pipeline path exists
  // here) -- the real two-way branch (Company Leads vs Pipeline) is
  // unchanged.
  const backTo = location.pathname.startsWith('/dashboard/company-leads') ? '/dashboard/company-leads' : '/dashboard/pipeline';
...
function AllocationEligibilitySection({ lead, config, payments }: { lead: Lead; config: NonNullable<ReturnType<typeof useConfig>['data']>; payments: Payment[] }) {
  ...
  async function request() {
    ...
    try {
      ...
      navigate('/dashboard/allocations');
    } catch (e) {
    ...
    <div className={styles.section}>
      ...
      {existing ? (
        <p className={styles.helpText}>
          ...
          <button type="button" className={styles.editLink} onClick={() => navigate('/dashboard/allocations')}>
            View in Allocations →
          </button>
        </p>
      ) : dep.complete ? (
    ...
    <div className={styles.section}>
      <div className={styles.sectionHeadRow}>
        ...
        <button
          ...
          onClick={() => navigate(`/dashboard/site-visits/new?leadId=${lead.id}&name=${encodeURIComponent(lead.name)}&contact=${encodeURIComponent(lead.contact)}`)}
        >
          Log a visit
        </button>
      </div>
