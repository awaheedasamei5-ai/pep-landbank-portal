...
// `section` narrows to a real inventory block (Plot Inventory's own
// "Block A"/"Block B"/... grouping, plots.section) -- real user ask: the
// AI suggestion should ask which section(s) to search before suggesting,
// not silently search the whole company-wide inventory every time.
function candidatesFor(plots: Plot[], plotType: PlotType, site: string | undefined, section: string | undefined, exclude: Set<string>, std: StandardDimensions): PlotSuggestion[] {
  return plots
    .filter((p) => p.status === 'Available' && p.plotType === plotType && !exclude.has(p.id) && (!site || p.site === site) && (!section || p.section === section))
...
function splitFallbackCandidatesFor(plots: Plot[], site: string | undefined, section: string | undefined, exclude: Set<string>): PlotSuggestion[] {
  return plots
    .filter((p) => p.status === 'Available' && p.plotType === 'Full Plot' && p.unitKind !== 'half' && !p.parentPlotId && !exclude.has(p.id) && (!site || p.site === site) && (!section || p.section === section))
...
export function suggestAlternatives(plots: Plot[], plotType: PlotType, std: StandardDimensions, site?: string, section?: string): PlotSuggestion[] {
  return candidatesForUnit(plots, plotType, site, section, new Set(), std).slice(0, 3);
}
...
export function suggestSet(plots: Plot[], units: PlotType[], std: StandardDimensions, site?: string, section?: string): (PlotSuggestion | null)[] {
